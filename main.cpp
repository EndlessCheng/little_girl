...
int getpos(char *ss, int p)
{
	mem(vis, 0);
	int i, len;
	for (i = 1; ss[i]; ++i) if (!vis[ss[i] & 31]) vis[ss[i] & 31] = true, Qmin(st[ss[i] & 31], i);
	mem(vis, 0), len = i;
	rForr(i, len - 1, 1) if (!vis[ss[i] & 31]) vis[ss[i] & 31] = true, Qmax(en[ss[i] & 31], i);
	return len;
}

int dp[mx][mx];

inline int U(int pos, int p)
{
	int c = 0;
	if (pos == en[s[p][pos] & 31]) ++c;
	else if (pos == st[s[p][pos] & 31]) --c;
	return c;
}
...
int f(int i, int j, int cnt)
{
	...
	if (j == 0) return dp[i][j] = cnt + f(i - 1, j, cnt + U(i, 0));
