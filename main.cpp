...
char t[mx], p[mx]; /// t是文本字符串，p是模板字符串
int f[mx]; /// 失配函数，可用来寻找与前缀相匹配的子串 (荐CF126B CF182D)

int get_fail(char *p) /// 返回p的长度，p是模板字符串
{
 f[1] = 0;
 int i, j;
 for (i = 1; p[i]; ++i)
...
void find(char *t, char *p) /// t是文本字符串，p是模板字符串
{
 int j = 0, m = get_fail(p);
 for (int i = 0; t[i]; ++i)
 {
  while (j && p[j] != t[i]) j = f[j];
  if (p[j] == t[i]) ++j;
  if (j == m) PI(i - m + 1); /// 找到了
  /// 或者写成: if (j == m) ans.PB(i - m + 1); /// ans为所有匹配点的下标
 }
}
