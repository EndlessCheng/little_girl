...
bool ok(int n)
{
   // PII(a[0],a[1]);
	...
	if (a[0] == inf)
	{
		if (a[1] == inf) return puts("none"), false;
		Qmin(up, a[1] - 1);
	}
	else if(a[1]!=inf)
	{
		if (a[1] < a[0]) return puts("none"), false;
	}

	if (a[n - 1] == inf)
	{
		if (a[n - 2] == inf)  return puts("none"), false;
		if ((n - 1) & 1)  Qmax(low, a[n - 2] + 1);
		else Qmin(up, a[n - 2] - 1);
	}
	else if(a[n-2]!=inf)
	{
		...
		if ((n - 1) & 1)
		{
			if (a[n - 1] < a[n - 2])return puts("none"), false;
		}
		else
		{
			if (a[n - 1] > a[n - 2])return puts("none"), false;
	...
	Forr(i, 1, n - 1)
	{
		if (a[i] == inf)
		{
			if (a[i - 1] == inf || a[i + 1] == inf) return puts("none"), false;
			if (i & 1)  Qmax(low, max(a[i - 1], a[i + 1]) + 1);
			else Qmin(up, min(a[i - 1], a[i + 1])  - 1);
		}
		else
		{
			if (a[i - 1] != inf)
			{
				if ((i) & 1)
				{
					if (a[i ] < a[i - 1])return puts("none"), false;
				}
				else
				{
					if (a[i ] > a[i - 1])return puts("none"), false;
...
int main()
{
	...
	while (SI(n), n)
	{
		...
		if (ok(n))
		{
			//PII(low, up);
			if (up == low) PI(up);
			else puts(up > low ? "ambiguous" : "none");
		}
	}
	return 0;
}
