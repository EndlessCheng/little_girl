...
//const int sqrt_mx = Sqrt(mx); /// 避免浮点误差
//
//bool vis[mx];
//int prime[mx], cnt;
//
...
//{
// int i, j;
// Forr(i, 2, sqrt_mx + 1) if (!vis[i]) {prime[++cnt] = i; Forrr(j, i * i, mx, i) vis[j] = true;}
// Forr(i, sqrt_mx + 1, mx) if (!vis[i]) prime[++cnt] = i;
//}
...
{int n,k,i,st;
   // getPrime();
    ...
    {
        st=1+6*i;
        PIIII(st*k,(st+1)*k, (st+2)*k,(st+4)*k);
    }
    return 0;
}
