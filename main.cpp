...
bool in[mx], now[mx];
int id[mx], n, m, outcache;
set<int> bf, ex, af, ans,ans2;

void before()
{
	mem(now, 1),outcache=-1;
}

void exact()
{
	mem(now, 0),outcache=-1;
	int i;
	For(i,m)
	{
	    if(in[i])
        ...
        {
            if(ex.find(id[i])==ex.end()) {ex.clear();return;}
...
int main()
{
	...
	while (~SII(n, m))
	{
		bf.clear(), ex.clear(), af.clear(), ans.clear(), ans2.clear();
		For(i, m) SC(message), in[i] = (message == '+'), SI(id[i]);
		before();
		exact();
		after();
		setInter(bf,ex,ans);
		setInter(af,ans,ans2);
	}
	return 0;
}
